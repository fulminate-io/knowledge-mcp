...
| `callee_depth` | number |  |  | How many levels of CALLEES to walk for query(graph='code', id=...) analyze. Clamped to 1..3 on the same rule as caller_depth. |
| `caller_depth` | number |  |  | How many levels of CALLERS to walk for query(graph='code', id=...) analyze. Clamped to 1..3: a value at or below zero reads as 1, anything above 3 reads as 3. |
