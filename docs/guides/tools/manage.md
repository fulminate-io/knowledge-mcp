...
| `rebuild_cache` | `graph: "code"`, `name` | Re-derive a repo's summary/embed caches (free, no model calls). |
| `rebuild_segments` | `graph: "code"`, `name` | Rebuild BM25+HNSW search segments from embedded nodes. |
...
| `dry_run` | boolean |  |  | For promote_metadata: when true, run the decision pass and report intended actions without mutating the graph. Default false (executes flips). |
