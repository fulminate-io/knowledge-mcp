...
| `graph` | string |  |  | Target graph: 'knowledge' (default), 'code' (requires repo), 'cloud' / 'cicd' (require account), 'practice' (requires language), 'transformers', or 'checks'. checks is a singleton and takes neither language nor name. |
