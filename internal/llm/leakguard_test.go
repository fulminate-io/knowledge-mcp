...
//
// This package's goroutines are TEST-side: service_test.go drives the service
// concurrently to exercise provider selection under parallel load. What the guard
// protects is a driver goroutine outliving its test while still holding a provider
// client, and with it whatever transport that client opened.
//
