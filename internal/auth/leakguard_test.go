...
//
// startLoopbackServer spawns the loopback HTTP server that receives the OAuth
// redirect (browser_flow.go:278). A test that starts the flow and does not complete
// or cancel it leaves that server goroutine holding a listening socket, which the
// next test binding the same port sees as an unexplained failure.
//
