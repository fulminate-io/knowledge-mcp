	...
	return kgtools.MCPTool{
		...
		InputSchema: kgtools.InputSchema{
			...
			Properties: map[string]kgtools.Property{
				...
				"where": {Type: "object", Description: "Optional JSON where-tree filter. Composers: all/any/not. Leaves: kind (node-kind constraint), matches (regex), equals (literal text), same_node (capture identity), same_text (capture text equality), flows_to (intra-declaration dataflow: from/to capture refs scoped to the declaration named by the REQUIRED within), inside_pattern + contains_pattern (recursive sub-patterns over ancestors / descendants; each accepts an optional 'as' field that names the matched ancestor/descendant for downstream sibling leaves to reference — useful for 'find ancestor function then check its body' shapes). Capture references: bare names (\"X\") for local user-named captures, \"$match\" for the outermost matched node (built-in, no explicit placeholder needed), \"$outer.X\" walks one parent scope and the whole token chains for deeper nesting (\"$outer.$outer.X\" is two levels), and \"<as>.<capture>\" reaches a sub-pattern leaf's own capture through the namespace that leaf declared in its 'as' field. See description block for examples.", AdditionalProperties: &falseValue, Properties: map[string]kgtools.Property{
					...
					"flows_to":         {Type: "object", Description: "Leaf: capture A flows to capture B inside the declaration named by within; e.g. { from: X, to: Y, within: FN }."},
				}},
...
An INLINED LITERAL CONSTRAINS BY VALUE. A string, character or comment written into a pattern must match the target's byte for byte: 'seed("checks/go", $N)' reaches calls whose first argument is that exact string and no others. This holds for every grammar, including the ones whose parse leaves a literal's content in a byte-span gap no child node covers (Go's interpreted string, Python's and OCaml's escaped strings, Kotlin's character literal, Scala's interpolated string, Bash's heredoc body, and the block-comment forms of Rust, Scala, Groovy and Elm). A placeholder cannot bind INSIDE one of those — see the compile-time refusals below — so capture the whole literal and constrain it with a where-tree leaf when you need a pattern over part of its text.
...
Capture references inside where-tree leaves come in three forms:
  ...
  - "$outer.X"       — capture from one parent scope (used inside sub-pattern wheres). Chain the WHOLE token for deeper nesting: "$outer.$outer.X" walks two levels, one "$outer." per level.
  - "<as>.<capture>" — a sub-pattern leaf's own capture, exported under the name that leaf declared in its 'as' field (as:"T" over a pattern binding $C exports "T.C"). A leaf with no 'as' exports nothing.
...
Eight leaves:
  ...
  - flows_to        — { "from": "X", "to": "Y", "within": "FN" }   (X reaches Y by dataflow inside the declaration bound to "within"; all three are REQUIRED — name the scope with "$match" or an inside_pattern "as" binding)
...
Three compile-time refusals, all so that a call which could never have matched says so instead of answering a confident zero. An unknown where-tree 'kind' is rejected BEFORE the walk, with a near-miss suggestion drawn from that language's own node-kind vocabulary — the same vocabulary list_node_kinds prints — and an anonymous token gets told it is one rather than offered a spelling. In a patterns[] alternation, a member that fails to compile is reported in pattern_errors while the members that did compile still run; the singular 'pattern' form is unchanged and still fails hard. And a placeholder written INSIDE a literal whose text is compared whole — a Go interpreted string, a Scala interpolated string, a Bash heredoc body — is refused naming the placeholder and the kind, because such a placeholder binds to no node and the pattern could only ever count zero. Capture the WHOLE literal with a single placeholder and constrain it with a where-tree 'matches' or 'equals' leaf, or write $$ if you meant a literal dollar sign.`
