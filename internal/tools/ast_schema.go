	...
	return kgtools.MCPTool{
		...
		InputSchema: kgtools.InputSchema{
			...
			Properties: map[string]kgtools.Property{
				...
				"repo":             {Type: "string", Description: "Code graph name (defaults to the active code graph when exactly one is loaded; required when multiple are loaded)."},
...
const astToolDescription = `Structural AST search-and-replace across code graphs via a placeholder-template DSL paired with a recursive JSON boolean where-tree, working uniformly across every indexed language. Five ops:
  ...
  - replace: The WRITE counterpart to match — match → interpolate captures into a 'replacement' template → splice → re-parse gate. dry_run defaults TRUE (preview unified diffs + blast-radius, no write); set dry_run:false to apply atomically. Overlapping/nested matches in a file refuse that file whole; a rewrite that fails to re-parse is rejected (never written). Example: pattern 'defer $X.Close()' + replacement 'safeClose($X)'.
