...
func init() {
	...
	for arm, spec := range mutateArmRegistry {
		// The owner's accept-and-ignore ruling for `account`, the SAME applier
		// the query registry runs so one parameter cannot get two answers from
		// two surfaces (param_accounting_account_ruled_ignored.go). It runs
		// BEFORE the sorted rejected order is precomputed below: that order is
		// derived from the rejected set and is what the gate iterates, so
		// applying the ruling afterwards would leave a stale key in the
		// deterministic first-hit list and the gate would still refuse the param
		// it no longer rejects.
