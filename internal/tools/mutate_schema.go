	...
	return kgtools.MCPTool{
		...
		InputSchema: kgtools.InputSchema{
			...
			Properties: map[string]kgtools.Property{
				...
				"references":            {Type: "array", Description: "Citations for findings: [{url, title}, {file, title}, or {node_id, title}]", Items: &kgtools.Property{Type: "object", Description: "Reference object"}},
				...
				"links":           {Type: "array", Description: "Node IDs to link the new thought to via informed-by (mutate(create, type=thought) only).", Items: &kgtools.Property{Type: "string"}},
				"session":         {Type: "string", Description: "Session name to group the thought under (mutate(create, type=thought) only). Creates the session if new."},
