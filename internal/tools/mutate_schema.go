...
func mutateProperties() map[string]kgtools.Property {
	props := map[string]kgtools.Property{
		...
		"language":              {Type: "string", Description: "LEGACY read-only practice selector naming one of the pre-singleton practice graphs. Every mutate arm is a write, so practice REFUSES it here: practice is one combined graph, and a create groups its node under an origin with source_hub instead."},
