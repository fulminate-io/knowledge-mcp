...
// parity_graph_fixture_test.go holds the in-memory graph fixture five test files
// in this package share, lifted out of intercept_query_plan_tree_test.go which
// had reached the repository's 500-line cap.
//
// THE LIFT WAS OVERDUE RATHER THAN INCIDENTAL. The fixture is not plan_tree's: it
// answers the four wire shapes any client read issues — a by-id node, a bulk
// ids[] hydrate, a bounded edges read and a bounded-depth traversal — and it is
// reached from the plan_tree tests, the section-order tests, the create_plan tree
// test and the annotation tests. Keeping it inside one caller's file made it look
// like that caller's private helper, which is part of why two of its arms went
// years without carrying edge metadata and one had no bulk-hydrate arm at all.
//
// EVERY ARM MUST ANSWER LIKE THE WIRE, and the two comments below say where that
// was not true and what it cost: a fixture that drops the field under test makes
// the assertion vacuous, and a fixture with a MISSING arm answers empty, which
// reads downstream as "the graph holds nothing" rather than as a fixture gap.
...
func (g *parityCaller) Execute(_ context.Context, req *knowledgev1.ExecuteRequest) (*knowledgev1.ExecuteResponse, error) {
	...
	// THE BULK ids[] HYDRATE, which this fixture had no arm for at all. Every
	// set-form hydrate therefore answered EMPTY, and an empty hydrate reads
	// downstream as "the graph holds nothing" rather than as a fixture gap — which
	// is how a plan_tree annotation read could return nothing against a fixture
	// that plainly held annotations.
	if ids := q.GetIds(); len(ids) > 0 {
		out := make([]*knowledgev1.Node, 0, len(ids))
		for _, id := range ids {
			if g.f.tombstoned[id] {
				continue
			}
			if n, ok := g.f.nodes[id]; ok {
				out = append(out, n)
			}
		}
...
func (g *parityCaller) answerTraversal(q *knowledgev1.QueryPlan) *knowledgev1.ExecuteResponse {
	...
	// The index holds the EDGE, not just the child id, because the walk below
	// must hand back every field the wire carries.
	childrenOf := map[string][]*knowledgev1.Edge{}
	for _, e := range g.f.edges {
		if e.Type == string(kgtypes.EdgeKGContains) {
			childrenOf[e.FromId] = append(childrenOf[e.FromId], e)
		}
	}
	...
	for len(queue) > 0 {
		...
		for _, e := range childrenOf[cur.id] {
			...
			// THE WALKED EDGE CARRIES ITS METADATA, not just the three identity
			// fields. This arm stands in for the wire, where engine.EdgesFromProto
			// copies Evidence and Method verbatim — and Evidence is where a
			// positioned child's `position` rides. A fixture that rebuilt the edge
			// from FromId/ToId/Type would silently strip the ordering key, and
			// every sibling-order assertion taken through it would read as
			// "position ignored" while passing against a correct implementation.
...
func (g *parityCaller) nodeSetEdges(ids, edgeTypes []string, outgoingOnly bool) []*knowledgev1.Edge {
	...
	for i := range g.f.edges {
		...
		// THE EDGE METADATA RIDES, not just the three identity fields. This arm
		// stands in for the wire, where engine.EdgesFromProto copies Method and
		// Evidence verbatim — and an annotation edge's kind and tier live on
		// Evidence. A fixture that rebuilt the edge from FromId/ToId/Type would
		// strip exactly the field a severity assertion reads.
		out = append(out, &knowledgev1.Edge{
			FromId:        e.FromId,
			ToId:          e.ToId,
			Type:          e.Type,
