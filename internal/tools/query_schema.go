...
// length budget.
const queryToolDescription = "Unified query tool. Search knowledge, code, practice, linkage, or a registered custom graph. " +
	...
	"mode='pivot' with rows=<key> cols=<key> → row×col matrix of node counts; " +
	"mode='correlations' → every CORRELATES_WITH edge in the graph, sorted by score desc; " +
	"mode='timeline' orders nodes by a time field; " +
	"mode='explain' (id=<node> or extra={a:<nodeA>, b:<nodeB>}) → per-correlation breakdown. " +
	...
	"mode='style_index' (graph='practice') renders the STYLE-RULE INDEX for one language hub: " +
	"one tab-separated line per rule — id, severity, scope, summary, sister-check id — and nothing else, " +
	"narrowed by source=<hub id> plus the optional repo=<repo> and path_prefix/path_prefixes the work touches. " +
	"A rule carrying no scope applies everywhere and is always listed; a rule whose scope excludes the repo or " +
	"every named path is absent. It DRAINS rather than pages, so it takes no limit/offset. " +
	"mode='metadata_stats' returns a per-graph cardinality histogram for every metadata key " +
	"(graph='practice' takes no instance selector and is one " +
	"combined graph, optionally narrowed by source=<hub id>; " +
	"graph='knowledge' or empty uses the default knowledge graph). " +
	"mode='correlations' applies a top-by-confidence cap of 100 rows, " +
	...
	"mode='timeline' returns the earliest 500 nodes by time, widenable with 'limit' up to 5000. " +
	...
	return kgtools.MCPTool{
		...
		InputSchema: kgtools.InputSchema{
			...
			Properties: map[string]kgtools.Property{
				...
				"mode":                {Type: "string", Description: "Search mode or special operation", Enum: []string{"hybrid", "text", "stats", "examine", "file_symbols", "modules", "personality", "influence", "tensions", "blind_spots", "evolution", "summary", "simulate", "timeline", "charges", "clusters", "recent", "topology", "pivot", "correlations", "explain", "lineage", "evidence", "plan_tree", "metadata_stats", "style_index"}},
				"rows":                {Type: "string", Description: "Row label key for mode='pivot' (e.g. 'type'). Defaults sniffed from the graph when omitted."},
				"cols":                {Type: "string", Description: "Column label key for mode='pivot' (e.g. 'status'). Defaults sniffed from the graph when omitted."},
				...
				"fields":              {Type: "array", Description: "Field projection (format=json only): list of fields to include per result, dramatically shrinking response size for high-volume queries. Top-level keys accepted on EVERY read: content, created_at, description, file_path, id, keywords, language, line, metadata, name, signature, source, status, summary, symbol_name, test_kind, tombstoned_at, type, updated_at. Three more — score, graph, graph_instance — are properties of a search HIT, so they are accepted ONLY on ranked-search reads (a call carrying text, or mode:\"text\"); naming one on a by-id or browse read is REFUSED with a message saying where the key IS available. Per-metadata-key: 'metadata.<key>' (e.g. 'metadata.dsl_pattern'). Bare 'metadata' includes the whole metadata map. Empty/absent = full hydration. An unsupported key is REFUSED, naming the offending key and the accepted vocabulary. A named top-level key is ALWAYS returned when you request it — empty string for an unset text field, 0 for an unset timestamp, empty map for absent metadata — so \"present and unset\" stays distinguishable from \"not in your projection\". tombstoned_at is the ONE exception to that always-returned rule: it is served only under the include_tombstones opt-in, naming it without the flag is REFUSED, and under the flag it is OMITTED ENTIRELY for a live node rather than returned as 0 — a sentinel 0 is indistinguishable at the wire from a real tombstone stamp. created_at and updated_at project as RAW int64 unix nanos, and an unset stamp returns 0 on these projection arms — UNLIKE mode:\"examine\" and mode:\"plan_tree\", which omit the key entirely when the stamp is zero. A per-metadata-key projection is OMITTED ENTIRELY from any result whose node lacks that key — absent from the row, never an empty string. So zero projected values across a result set means the key is unset on those nodes; it is NOT evidence that a write failed.", Items: &kgtools.Property{Type: "string"}},
				...
				"time_field":          {Type: "string", Description: "Node field or metadata key to use as timestamp for timeline mode (e.g. CreatedAt, UpdatedAt, or a metadata key). "},
				"format":              {Type: "string", Description: "Output format: 'text' (default) or 'json' (structured). Recognized by several modes. A `fields` PROJECTION OVERRIDES IT: when fields is supplied the read emits the json projection whatever format says, on the by-id, ids-hydrate and plan_tree arms alike — a projected row is a json object, so there is no text shape to render it into. Measure format's effect on a read carrying NO fields."},
				...
				"samples":             {Type: "boolean", Description: "Add bounded per-type sample node names to a mode='stats' render. Honored by every stats arm: knowledge, practice/linkage, code, and the registered custom types."},
