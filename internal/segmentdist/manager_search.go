...
// batched Fetch for the delta, parallel Import) before the search. The two
// engine.Search calls run CONCURRENTLY over a bounded WaitGroup (the engines are
// independent and each is internally per-segment parallel), not serially.
//
