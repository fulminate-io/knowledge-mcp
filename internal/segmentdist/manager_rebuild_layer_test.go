...
//
// Both tests here drive a real Manager over the sharedServerFake, which models
// per-writer manifests and the refcount-GC, so the published id-set, the surviving
// blob set and the per-digest doc counts are all directly readable.
...
// hnswManifestDigests returns the published hnsw manifest for one writer as
// id -> doc count, read off the server model rather than off any engine: the
// question every assertion below asks is what the SERVER now references, which an
// engine's own Export cannot answer.
func hnswManifestDigests(svc *sharedServerFake, target *knowledgev1.GraphSelector, writerID string) map[string]int {
	out := map[string]int{}
	for _, m := range svc.manifestMetas(target, writerID) {
		if m.GetFormat() == hnswFormatName {
			out[m.GetId()] = int(m.GetDocCount())
		}
	}
	return out
}

// storedBlobIDs is the set of blob ids the server still holds for a target — what
// survived the refcount-GC the last publish drove.
func storedBlobIDs(svc *sharedServerFake, target *knowledgev1.GraphSelector) map[string]struct{} {
...
// cycle A's segments PLUS cycle B's; publishResident then computed
// dropped = reconcileAgainst − liveSet with liveSet a SUPERSET of shippedIDs, so the
...
func TestResetRebuildPublishesOnlyThisRunsLayer(t *testing.T) {
	...
	layerA := hnswManifestDigests(svc, target, "")
	...
	layerB := hnswManifestDigests(svc, target, "")
	...
	require.NotEmpty(t, superseded,
		"%s: the reset publish must report cycle A's segments as superseded; an empty set is liveSet ⊇ shippedIDs",
...
func TestRebuildDeltaReEmitsOwningBucketOnly(t *testing.T) {
	...
	before := hnswManifestDigests(svc, target, "")
	...
	after := hnswManifestDigests(svc, target, "")
