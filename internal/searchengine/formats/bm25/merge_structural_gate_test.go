...
// realMergeEmitterState runs the genuine merge machinery over one sealed input
// and returns the emitter it used alongside the length the merge would have
// reported.
//
// The machinery it runs is realMergeMachinery (merge_armorder_test.go), which
// replicates rather than calls streamMergeToFile because both the emitter and
// the writer are internal to that function. Keeping ONE copy of that sequence in
// the package's tests is what stops two replicas of it drifting apart from
// streamMergeToFile in different directions; the gate's inputs are exactly the
// emitter's accumulated state, and this wrapper supplies the fixture.
func realMergeEmitterState(t *testing.T, kind byte) (*mergeEmitter, int64) {
	...
	e, w := realMergeMachinery(t, []*mappedSegment{seg}, acceptEvery(1), kind, sink)
	return e, w.tail
}
