...
// bind-first daemon binds the HTTP MCP listener first, then wires the worker /
...
// markWorkerReady / markPropReady / markPipelineReady latch the corresponding
...
// (c.runtime / c.propLoop / c.pipeline) is assigned, so the atomic Store
// publishes the fully-wired handle to any reader that observes Ready()==true.
func (c *client) markWorkerReady()   { c.workerReady.Store(true) }
...
// background wiring stage has completed. False during the bind-first wiring
// window (bind-first startup); the intercept guards consult these to emit a "daemon still
...
func (c *client) WorkerReady() bool   { return c.workerReady.Load() }
