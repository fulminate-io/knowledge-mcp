...
// analyzers — HITSHubAnalyzer and HITSAuthorityAnalyzer — over gonum's
// network.HITS algorithm.
//
...
// graph, runs network.HITS, and projects the per-node HubAuthority pair
// down to whichever score the caller asked for. The selector closure +
...
// computeHITSScores materializes the graph, runs network.HITS, and
// returns the HITS score map plus the int→string ID mapping. Returns
// (nil, nil, nil) on empty-graph input so the caller short-circuits
// cleanly.
func computeHITSScores(ctx context.Context, req foundation.Request, name string, tolerance float64) (map[int64]network.HubAuthority, map[int64]string, error) {
	...
	// Empty graph: gonum's HITS divides by a 2-norm computed over a
	// zero-length vector and panics. Guard before calling.
	if g.Nodes().Len() == 0 {
		return nil, nil, nil
	}
	scores := network.HITS(g.WeightedDirectedGraph, tolerance)
