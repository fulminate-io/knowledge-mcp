...
//
// The exposure classifiers fan out hard: fanOutSGClassifiers spawns four AWS
// classifiers at once (aws_sg_reachability_findings.go:66-69) and the Kubernetes
// side spawns more still (k8s_reachability_findings.go). They coordinate on a
// WaitGroup, so a classifier that panics or returns early without Done leaves its
// siblings — and the waiter — parked forever.
//
