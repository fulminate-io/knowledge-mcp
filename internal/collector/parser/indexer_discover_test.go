...
func TestDiscoverFiles(t *testing.T) {
	...
	// Generated Go files. .pb.go is skipped on suffix alone; the
	// _gen.go/_generated.go suffixes skip ONLY when the file carries the Go
	// generated-code marker — a hand-written file that merely ends in _gen.go
	// is real source and must be discovered.
	writeFile(t, filepath.Join(dir, "api.pb.go"), "package api")
	writeFile(t, filepath.Join(dir, "model_gen.go"), "// Code generated by modelc. DO NOT EDIT.\npackage gen")
	writeFile(t, filepath.Join(dir, "model_generated.go"), "// Code generated by modelc. DO NOT EDIT.\npackage gen")
	writeFile(t, filepath.Join(dir, "pipeline_gen.go"), "package gen // hand-written despite the suffix")
	...
	if fileSet["model_gen.go"] {
		t.Error("expected model_gen.go to be excluded (carries the generated-code marker)")
	}
	if fileSet["model_generated.go"] {
		t.Error("expected model_generated.go to be excluded (carries the generated-code marker)")
	}
	if !fileSet["pipeline_gen.go"] {
		t.Error("expected hand-written pipeline_gen.go to be discovered — the _gen.go suffix alone is not evidence of generation")
	}
	for _, f := range files {
