...
func TestSummarize_Fallback_WithRegion(t *testing.T) {
	resetForTesting()
	t.Cleanup(resetForTesting)
	...
	assert.Equal(t, "made-up-type thing in us-east-1", got)
}
...
func TestSummarize_HelperReturnsEmpty_FallsBack(t *testing.T) {
	...
	got := Summarize(ResourceSpec{ResourceType: "empty-rt", Name: "x", Region: "r"})
	assert.Equal(t, "empty-rt x in r", got)
}
...
func TestBuildNode_PopulatesSummary(t *testing.T) {
	...
	Register("test-rt", func(spec ResourceSpec) string {
...
func TestBuildNode_PopulatesSummary_FallbackWhenUnregistered(t *testing.T) {
	...
	assert.Equal(t, "unregistered-rt anon in us-east-1", n.Summary)
}
