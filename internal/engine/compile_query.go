...
type queryArgs struct {
	...
	// nodes grouped under it. Omitted means the whole combined graph. It is not
	// a graph selector: the hub is a node inside the one practice graph, so this
	// lowers onto the `source_hub` metadata key rather than onto the wire
	// selector, and GraphSelector is byte-identical with and without it.
	...
	// AllTypes is the EVERY-NODE-OF-THIS-GRAPH browse: a type-less selection the
	// store serves as Match("") with no type filter.
	//
	// IT IS AN EXPLICIT OPT-IN KEY RATHER THAN "a query that set nothing", and
	// the difference is the whole reason it exists as a field. This compiler's
	// default-deny is load-bearing — a read that reduces to no recognized shape
	// falls through to the legacy wire rather than being served as something the
	// caller did not ask for — so a mode-less query with no id, type or meta must
	// keep returning ok=false. A caller that genuinely wants every node says so
	// by name, and gets the same Selection the meta-only arm already builds.
	//
	// ITS ONE CALLER is the collect-context fill's all_node_types drain: a family
	// declaring every node type has no list to iterate, and a per-type browse
	// cannot see a type nobody named.
...
func compileQuery(args json.RawMessage) (*knowledgev1.ExecuteRequest, bool) {
	...
	// graph=code id→the analyze-node intercept, text→the code-search intercept;
	// graph=logs id/text→the log engine. A plain type-browse /
...
// field — its presence routes the legacy tool to the recall surface, which is
// SPECIALIZED and which the engine does not serve.
...
func buildQueryPlan(a queryArgs) (*knowledgev1.QueryPlan, bool) {
	...
	// retired server-side generic-query precedence (ids → id → text → type →
	// meta-only).
...
func buildDefaultModePlan(a queryArgs) (*knowledgev1.QueryPlan, bool) {
	...
	case a.Type != "":
		...
		return p, true
	case a.AllTypes:
		// EVERY node of the target graph → Match("") with no type filter. It
		// carries the keyset cursor, because its only caller drains the whole
		// graph in bounded pages.
