...
<constraint id="domain-prior" severity="hard">
  ...
  <rule>
    Establish the project's DOMAIN SHAPE before proposing candidates: read the
    project's own description (CLAUDE.md / AGENTS.md / README) and recall any
    stored domain-fixture findings (`thoughts(recall, query:"domain fixtures
    architectural priors")`). Domain shape dictates architectural fixtures that
    are NOT anomalies to explain — a domain's central types will be load-bearing,
    its characteristic surfaces will be large, and its tooling will have known
    artifacts.
  </rule>

  <test>
    Before proposing any candidate touching a domain fixture, ask: "Would a
    knowledgeable engineer IN this domain immediately say 'yes, of course'?"
    If yes — drop. The candidate is explaining the domain to someone in the
    domain. When you derive a new domain fixture during a sweep, record it as a
    finding so future explorers recall it instead of re-deriving it.
  </test>
